...
wget -O "$WWW_DIR/country/subscriptions.txt" "$REPO_URL/country/subscriptions.txt"

# Загружаем CGI-скрипты
echo "Загружаем CGI-скрипты в $WWW_DIR/cgi-bin..."
wget -O "$WWW_DIR/cgi-bin/upload_subscriptions.sh" "$REPO_URL/cgi-bin/upload_subscriptions.sh"
wget -O "$WWW_DIR/cgi-bin/set_proxy.sh"           "$REPO_URL/cgi-bin/set_proxy.sh"
wget -O "$WWW_DIR/cgi-bin/countries.sh"           "$REPO_URL/cgi-bin/countries.sh"
